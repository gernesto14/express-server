...
import pool from "../config/db.js"; // import pg pool
  ...
  try {
    ...
    if (response.status === "OK") {
      ...
      try {
        ...
        // Save user profile
        await saveUserProfile(userId, validatedFormFields);
      } catch (error) {
        console.log(
          "❌ Error updating metadata or saving profile:",
          error.message
        );
      }
    }
...
export async function saveUserProfile(userId, data) {
  ...
  */
  ...
  console.log("\nℹ️ Saving user profile for user: ", userId);

  //    data:  {
  //   email: 'sdcslknc@klnsd.com',
  //   password: 'kjsbd78hidn',
  //   first_name: 'Iunkj',
  //   last_name: 'Nkj',
  //   access_code: 'demo',
  //   company_name: 'demo',
  ...
  const { email, first_name, last_name, access_code, company_name } = data;

  const query = `
    INSERT INTO users (id, email, first_name, last_name, access_code, company_name)
    VALUES ($1, $2, $3, $4, $5, $6)
  ...
  console.log("✅ User profile saved: ", rows[0]);
